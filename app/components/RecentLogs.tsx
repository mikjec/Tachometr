...
export function RecentLogs({ logs }: { logs: Log[] }) {
	return (
		<div className='bg-white rounded-2xl p-4 sm:p-6 shadow-sm border border-gray-100 block'>
			<h2 className='text-lg md:text-xl lg:text-2xl font-semibold text-gray-800 mb-4'>Ostatnie wpisy</h2>
									...
									<div className='flex flex-col gap-2 md:grid md:grid-cols-[1fr_1fr_3fr_1fr] p-4 md:px-4 md:py-3 rounded-xl hover:bg-gray-50 transition-colors md:items-center border border-gray-100 md:border-transparent'>
										<div className='flex justify-between items-center md:contents'>
											<span className='text-sm md:text-base lg:text-lg text-gray-700'>
												<span className='md:hidden text-[10px] text-gray-400 uppercase mr-2'>Data:</span>
											...
											<div className='md:hidden'>
												{log.paid ? (
													...
													<span className='px-2 py-1 rounded-full text-[10px] font-medium bg-yellow-100 text-yellow-700'>
														Oczekuje
													</span>
												)}
											</div>
										...
										<span className='text-sm md:text-base lg:text-lg font-bold text-gray-700 flex items-center'>
											<span className='md:hidden text-[10px] text-gray-400 uppercase font-normal mr-2'>Czas:</span>
											{log.hours}h
										</span>

										<span className='text-sm md:text-base lg:text-lg text-gray-500 truncate md:pr-4'>
											{log.note ?? '—'}
										</span>
